...
/-- Gets an array of all file paths in the project. -/
partial def allFilePaths : IO (Array FilePath) := do
  allFilePaths.go (#[] : Array FilePath) "." ("." : FilePath)

/-- Gets an array of all module names in the project. -/
def allPhysLeanModules : IO (Array Lean.Name) := do
  let paths ← allFilePaths
  let moduleNames := paths.map fun path =>
    let relativePath := path.toString.replace "./" ""
