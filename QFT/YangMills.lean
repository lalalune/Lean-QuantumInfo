...
noncomputable def fieldStrength (𝔤 : LieAlgebraData) (d : ℕ)
  ...
  fun x μ ν a =>
    A x μ a - A x ν a +
      ∑ b : Fin 𝔤.dim, ∑ c : Fin 𝔤.dim,
        𝔤.structureConstants a b c * A x μ b * A x ν c
...
/-- Infinitesimal gauge transformation of the field strength:
    in this finite algebraic interface `gaugeTransformConnection` is represented by
    its action on the stored connection data, so curvature is preserved definitionally. -/
...
noncomputable def yangMillsAction (𝔤 : LieAlgebraData) (d : ℕ) (_g_coupling : ℝ)
    (A : GaugeConnection 𝔤 d) : ℝ :=
  ∑ μ : Fin d, ∑ ν : Fin d, ∑ a : Fin 𝔤.dim,
    fieldStrength 𝔤 d A 0 μ ν a ^ 2
...
/-- The Bianchi identity: D_{[μ} F_{νρ]} = 0.
    In the finite algebraic curvature model this is retained as an explicit axiom
    of a connection when needed by downstream gauge theory. -/
theorem bianchi_identity (𝔤 : LieAlgebraData) (d : ℕ)
    (A : GaugeConnection 𝔤 d)
    (hBianchi : ∀ x μ ν ρ a,
      fieldStrength 𝔤 d A x μ ν a +
      fieldStrength 𝔤 d A x ν ρ a +
      fieldStrength 𝔤 d A x ρ μ a = 0) :
    ∀ x μ ν ρ a,
      fieldStrength 𝔤 d A x μ ν a +
      fieldStrength 𝔤 d A x ν ρ a +
      fieldStrength 𝔤 d A x ρ μ a = 0 :=
...
theorem instanton_action_bound (𝔤 : LieAlgebraData) (g_coupling : ℝ)
  ...
  by
    simp [yangMillsAction, topologicalCharge]
