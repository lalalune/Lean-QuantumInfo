...
-/
import QFT.AnomalyCancellation.PureU1.BasisLinear
import QFT.AnomalyCancellation.PureU1.VectorLike
/-!
...
lemma basis_on_evenFst_other {k j : Fin n.succ} (h : k ≠ j) :
  ...
  · split
    · rename_i h1 h2
      ...
      simp only [Fin.coe_castAdd, Fin.val_addNat] at h2
      omega
    · rfl
...
lemma basis!_evenShftSnd_eq_neg_evenShiftFst (j i : Fin n) :
  ...
  all_goals
    ...
    simp_all only [Fin.natAdd_eq_addNat, Fin.cast_inj, Fin.coe_cast, Fin.coe_natAdd,
      Fin.coe_castAdd, add_right_inj, Fin.val_addNat, add_eq_left]
  · subst h1
    exact Fin.elim0 i
  all_goals
    rename_i h3
    rw [Fin.ext_iff] at h3
    simp_all only [Fin.coe_natAdd, Fin.coe_castAdd, Fin.val_addNat, not_true_eq_false]
  all_goals
    omega
