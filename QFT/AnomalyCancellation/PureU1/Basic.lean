...
-- NOTE (`K3HYF`): Pure U(1) anomaly-cancellation conditions currently use
-- `ACCSystemCharges`. This directory could be simplified by defining
-- `PureU1Charges` directly as `Fin n → ℚ` (or a quotient by permutations
-- and overall factors).
