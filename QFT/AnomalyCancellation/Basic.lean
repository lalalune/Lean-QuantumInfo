...
-- NOTE: Replace `ACCSystemChargesMk` with `⟨n⟩` notation where practical.
/--
...
We give some open items for future work.
...
-- NOTE: Derive anomaly cancellation conditions from gauge/fermion data (for example via topological invariants).

-- NOTE: Add an algebraic-variety formulation and connect it to the approach here.
