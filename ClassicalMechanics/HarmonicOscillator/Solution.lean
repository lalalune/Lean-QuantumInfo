...
-- NOTE (`6VZME`): Implement other initial-condition encodings (for example:
-- two positions at different times, or two velocities at different times)
-- and convert them into `InitialConditions`.
...
for the given initial conditions. This is currently open future work.
...
We give some open items for the classical harmonic oscillator.

-/

-- NOTE (`6VZI3`): Find the time at which the classical harmonic oscillator
-- returns to its initial position and velocity.

-- NOTE (`6VZJB`): Find the times at which the classical harmonic oscillator
-- passes through zero.
