...
This module currently provides a lightweight formal foundation. The following results
are planned next:
...
-- NOTE: Prove that energy is not conserved and derive the energy dissipation rate.
-- NOTE: Derive solutions for the underdamped case (oscillatory with exponential decay).
-- NOTE: Derive solutions for the critically damped case (fastest non-oscillatory return).
-- NOTE: Derive solutions for the overdamped case (slow non-oscillatory return).
...
/-- The damped harmonic oscillator data structure.
...
/-- Quality factor `Q = m ω₀ / γ` for positive damping. -/
noncomputable def qualityFactor : ℝ := S.m * S.ω₀ / S.γ

/-- Relaxation time `τ = 2m/γ` for positive damping. -/
noncomputable def relaxationTime : ℝ := (2 * S.m) / S.γ

lemma qualityFactor_pos (hγ : 0 < S.γ) : 0 < S.qualityFactor := by
  unfold qualityFactor
  exact div_pos (mul_pos S.m_pos S.ω₀_pos) hγ

lemma relaxationTime_pos (hγ : 0 < S.γ) : 0 < S.relaxationTime := by
  unfold relaxationTime
  exact div_pos (mul_pos (by norm_num) S.m_pos) hγ

/-- With zero damping, the damped equation reduces to the undamped oscillator equation. -/
theorem equationOfMotion_gamma_zero (x : Time → ℝ) (hγ : S.γ = 0) :
    S.EquationOfMotion x ↔ ∀ t : Time,
      S.m * (Time.deriv (Time.deriv x) t) + S.k * x t = 0 := by
  constructor
  · intro hx t
    specialize hx t
    simpa [DampedHarmonicOscillator.EquationOfMotion, hγ, add_assoc, add_left_comm, add_comm] using hx
  · intro hx t
    specialize hx t
    simpa [DampedHarmonicOscillator.EquationOfMotion, hγ, add_assoc, add_left_comm, add_comm] using hx
