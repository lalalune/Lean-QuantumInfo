...
lemma velocity_const_of_zero_acc (q : Time → ℝ) (h : ∀ t, deriv (deriv q) t = 0)
    (hvel : Differentiable ℝ (deriv q)) : ∃ v₀, ∀ t, deriv q t = v₀ := by
  refine ⟨deriv q 0, fun t => ?_⟩
  exact is_const_of_deriv_eq_zero hvel h t 0
/--
...
theorem kineticEnergy_conserved (s : FreeParticle) (q : Trajectory)
    (h : ∀ t, s.NewtonsSecondLaw q t) (hvel : Differentiable ℝ (deriv q)) :
    ∃ E, ∀ t, s.kineticEnergy q t = E := by
  -- get q'' = 0
  have h_acc : ∀ t, deriv (deriv q) t = 0 :=
  accel_zero s q h
  -- get constant velocity
  rcases velocity_const_of_zero_acc q h_acc hvel with ⟨v₀, hv⟩
  -- energy is constant
