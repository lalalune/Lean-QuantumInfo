...
Note `EGU3E` may require considerable effort to be made rigorous and may heavily depend on
...
-- NOTE (`EGQUA`): Show that the wave equation is invariant under rotations and any direction `s`
-- can be rotated to `EuclideanSpace.single 2 1` when one wave is considered.
...
-- NOTE (`EGU3E`): Show that suitable disturbances can be expressed
