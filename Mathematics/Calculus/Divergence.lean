...
**Specialization**: `SpaceAndTime.Space.Derivatives.Div` provides a specialized `div` operator
for functions `Space d → EuclideanSpace ℝ (Fin d)`, with an associated distributional divergence.
That definition is specialized to the `Space d` type; this module provides the general version.
