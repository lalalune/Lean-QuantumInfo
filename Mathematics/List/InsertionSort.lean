...
instance instIsTotalOfStdTotal {α : Type} (r : α → α → Prop) [Std.Total r] : IsTotal α r :=
  ⟨Std.Total.total⟩
...
lemma insertionSortMin_lt_mem_insertionSortDropMinPos {α : Type} (r : α → α → Prop) [DecidableRel r]
  ...
  have hl1 : l1.Pairwise r := List.sorted_insertionSort r (a :: l)
  simp only [l1] at hl1
...
lemma insertionSort_insertionSort {α : Type} (r : α → α → Prop) [DecidableRel r]
  ...
  (List.sorted_insertionSort r l1).insertionSort_eq
...
lemma insertionSort_orderedInsert_append {α : Type} (r : α → α → Prop) [DecidableRel r]
  ...
  | b :: l1, l2 => by
    ...
    · simp only [List.orderedInsert, h, ↓reduceIte, List.cons_append, List.insertionSort]
      rw [insertionSort_orderedInsert_append r a l1 l2]
      simp only [List.insertionSort, List.cons_append]
      exact (orderedInsert_commute r a b h (List.insertionSort r (l1 ++ l2))).symm
...
lemma insertionSort_insertionSort_append {α : Type} (r : α → α → Prop) [DecidableRel r]
  ...
  | a :: l1, l2 => by
    simp only [List.insertionSort, List.cons_append]
    rw [insertionSort_orderedInsert_append r a (List.insertionSort r l1) l2]
    simp only [List.insertionSort, List.cons_append]
...
lemma insertionSortEquiv_orderedInsert_append {α : Type} (r : α → α → Prop) [DecidableRel r]
    ...
    (insertionSortEquiv r (List.orderedInsert r a l1 ++ a2 :: l2) ⟨l1.length + 1, by
      simp⟩)
    = (finCongr (by
      simp only [List.cons_append, orderedInsert_length, List.length_cons,
  ...
  | b :: l1, l2 => by
    ...
    · have h1 : (List.orderedInsert r a (b :: l1) ++ a2 :: l2) =
      ...
      simp only [List.insertionSort, List.cons_append, List.length_cons,
...
lemma insertionSort_filter {α : Type} (r : α → α → Prop) [DecidableRel r] [Std.Total r]
  ...
  | a :: l => by
    ...
    · rw [List.filter_cons_of_pos (by simp [hp])]
      simp only [List.insertionSort]
      rw [insertionSort_filter r p l]
      exact (orderedInsert_filter_of_pos r a p hp
        (List.insertionSort r l) (List.sorted_insertionSort r l)).symm
    · rw [List.filter_cons_of_neg (by simp [hp])]
      rw [insertionSort_filter r p l]
      exact (orderedInsert_filter_of_neg r a p hp (List.insertionSort r l)).symm
...
lemma filter_rel_eq_insertionSort {α : Type} (r : α → α → Prop) [DecidableRel r]
  ...
  | b :: l => by
    ...
    · have hl := orderedInsert_filter_of_pos r b (fun c => r a c ∧ r c a) h
        (List.insertionSort r l) (List.sorted_insertionSort r l)
