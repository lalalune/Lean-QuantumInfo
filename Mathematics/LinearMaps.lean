...
-/
-- NOTE (`6VZLZ`): Replace the definitions in this file with Mathlib definitions.
/-!
## Mathlib migration notes

The following replacements should be made when migrating:
- `HomogeneousQuadratic V` → `QuadraticMap ℚ V ℚ` (Mathlib's `QuadraticMap`)
- `BiLinearSymm V` → A symmetric `BilinMap ℚ V ℚ` (via `LinearMap.BilinForm`)
- `HomogeneousCubic V` → No direct Mathlib equivalent; keep or build on `MultilinearMap`
- `TriLinearSymm V` → A symmetric `MultilinearMap ℚ (Fin 3 → V) ℚ` with symmetry proof

The migration is blocked on `QFT.AnomalyCancellation.Basic` and downstream Particles/ files
that depend on the specific API surface (especially `mk₂`, `mk₃`, `toCubic`, `toHomogeneousQuad`).
-/
