...
lemma IsTestFunction.space_component {φ : X → Space d} (hφ : IsTestFunction φ) :
  ...
  smooth := by
    exact ContDiff.comp (Space.eval_contDiff (n := ∞) i) hφ.smooth
  supp := by
...
lemma IsTestFunction.coord {φ : X → Space d} (hφ : IsTestFunction φ) (i : Fin d) :
    IsTestFunction (fun x => (φ x).coord i) := by
  simpa [Space.coord_apply] using (IsTestFunction.space_component (φ := φ) (i := i) hφ)
...
/-- The divergence of a test function (vector field) is a test function.

**Proof strategy:** divergence f = Σᵢ ∂ᵢ(f·eᵢ) in a basis. Each term is a derivative
of a test function composed with a linear map, hence is a test function.
**Missing Mathlib API:** The fun_prop is at `IsTestFunction.comp_left` applied to
`(fderiv ℝ f ·)` composed with `LinearMap.trace`. Need either:
- `HasCompactSupport` for the map x ↦ trace (fderiv ℝ f x)
- Or `ContDiff` for LinearMap.trace ∘ fderiv
- Or a dedicated lemma: test functions are preserved under taking divergence
  (trace of Jacobian). The divergence_eq_sum_fderiv' expansion suggests we need
  `IsTestFunction.of_fderiv` and a statement that trace of a test-function-valued
  linear map is a test function. -/
@[fun_prop]
