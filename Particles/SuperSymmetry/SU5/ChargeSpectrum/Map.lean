...
private lemma add_toFinset_image [DecidableEq 𝓩]
    (f : 𝓩 →+ 𝓩1) {a b : Multiset 𝓩} {a' b' : Multiset 𝓩1}
    (ha : a'.toFinset = a.toFinset.image f) (hb : b'.toFinset = b.toFinset.image f) :
    ((a' ×ˢ b').map (fun y : 𝓩1 × 𝓩1 => y.1 + y.2)).toFinset =
      (((a ×ˢ b).map (fun y : 𝓩 × 𝓩 => y.1 + y.2)).toFinset).image f := by
  ext z
  constructor
  · intro hz
    rcases Multiset.mem_toFinset.mp hz with hz
    rcases Multiset.mem_map.mp hz with ⟨y, hy, rfl⟩
    rcases Multiset.mem_product.mp hy with ⟨hy1, hy2⟩
    have hy1' : y.1 ∈ a.toFinset.image f := by
      rw [← ha]
      exact Multiset.mem_toFinset.mpr hy1
    have hy2' : y.2 ∈ b.toFinset.image f := by
      rw [← hb]
      exact Multiset.mem_toFinset.mpr hy2
    rcases Finset.mem_image.mp hy1' with ⟨x1, hx1, hx1eq⟩
    rcases Finset.mem_image.mp hy2' with ⟨x2, hx2, hx2eq⟩
    refine Finset.mem_image.mpr ⟨x1 + x2, ?_, by simpa [map_add, hx1eq, hx2eq]⟩
    apply Multiset.mem_toFinset.mpr
    apply Multiset.mem_map.mpr
    refine ⟨(x1, x2), Multiset.mem_product.mpr ?_, rfl⟩
    exact ⟨Multiset.mem_toFinset.mp hx1, Multiset.mem_toFinset.mp hx2⟩
  · intro hz
    rcases Finset.mem_image.mp hz with ⟨y, hy, rfl⟩
    rcases Multiset.mem_toFinset.mp hy with hy
    rcases Multiset.mem_map.mp hy with ⟨x, hx, rfl⟩
    rcases Multiset.mem_product.mp hx with ⟨hx1, hx2⟩
    have hx1' : f x.1 ∈ a'.toFinset := by
      rw [ha]
      exact Finset.mem_image.mpr ⟨x.1, Multiset.mem_toFinset.mpr hx1, rfl⟩
    have hx2' : f x.2 ∈ b'.toFinset := by
      rw [hb]
      exact Finset.mem_image.mpr ⟨x.2, Multiset.mem_toFinset.mpr hx2, rfl⟩
    apply Multiset.mem_toFinset.mpr
    apply Multiset.mem_map.mpr
    refine ⟨(f x.1, f x.2), Multiset.mem_product.mpr ?_, ?_⟩
    · exact ⟨Multiset.mem_toFinset.mp hx1', Multiset.mem_toFinset.mp hx2'⟩
    · simp [map_add]

private lemma foldl_ofFieldLabel_toFinset_image [DecidableEq 𝓩]
    (f : 𝓩 →+ 𝓩1) (x : ChargeSpectrum 𝓩) :
    ∀ (L : List FieldLabel) (acc : Multiset 𝓩) (acc' : Multiset 𝓩1),
      acc'.toFinset = acc.toFinset.image f →
      (List.foldl
          (fun a b : Multiset 𝓩1 => (a ×ˢ b).map (fun y : 𝓩1 × 𝓩1 => y.1 + y.2))
          acc'
          (L.map fun F => (ofFieldLabel (map f x) F).val)).toFinset
        =
      (List.foldl
          (fun a b : Multiset 𝓩 => (a ×ˢ b).map (fun y : 𝓩 × 𝓩 => y.1 + y.2))
          acc
          (L.map fun F => (ofFieldLabel x F).val)).toFinset.image f := by
  intro L
  induction L with
  | nil =>
      intro acc acc' hacc
  ...
  | cons F Fs ih =>
      intro acc acc' hacc
      have hF :
          ((ofFieldLabel (map f x) F).val).toFinset =
            ((ofFieldLabel x F).val).toFinset.image f := by
        rw [map_ofFieldLabel]
        ext y
        simp [Finset.mem_image, Multiset.mem_map]
      have hnext := add_toFinset_image (f := f)
        (a := acc) (b := (ofFieldLabel x F).val)
        (a' := acc') (b' := (ofFieldLabel (map f x) F).val) hacc hF
      simpa [List.foldl_cons, List.map_cons] using
        ih
          ((fun a b : Multiset 𝓩 => (a ×ˢ b).map (fun y : 𝓩 × 𝓩 => y.1 + y.2))
            acc ((ofFieldLabel x F).val))
          ((fun a b : Multiset 𝓩1 => (a ×ˢ b).map (fun y : 𝓩1 × 𝓩1 => y.1 + y.2))
            acc' ((ofFieldLabel (map f x) F).val))
          hnext
...
lemma map_ofPotentialTerm_toFinset [DecidableEq 𝓩]
  ...
  simpa [ofPotentialTerm] using
    foldl_ofFieldLabel_toFinset_image (f := f) (x := x)
      T.toFieldLabel ({0} : Multiset 𝓩) ({0} : Multiset 𝓩1) (by simp)
