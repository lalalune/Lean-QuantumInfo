...
/-- Z_on_qubit2 represents a nontrivial logical coset of weight 1. -/
lemma Z_on_qubit2_represents_nontrivial_coset :
    RepresentsNontrivialCoset Z_on_qubit2 stabilizerGroup := by
  refine ⟨Z_on_qubit2_mem_centralizer, ?_, ?_⟩
  · rw [stabilizerGroup_toSubgroup_eq]
    exact Z_on_qubit2_not_mem_subgroup
  intro s hs hs_ops
  have hs_anticomm : NQubitPauliGroupElement.Anticommute s logicalX := by
    rw [NQubitPauliGroupElement.anticommutes_iff_odd_anticommutes]
    rw [hs_ops]
    exact (NQubitPauliGroupElement.anticommutes_iff_odd_anticommutes
      Z_on_qubit2 logicalX).mp Z_on_qubit2_anticommutes_logicalX
  exact not_mem_stabilizer_of_anticommutes_centralizer stabilizerGroup s logicalX
    logicalX_mem_centralizer hs_anticomm hs
...
theorem repetitionCode3_has_distance_one : HasCodeDistance stabilizerCode 1 := by
  refine ⟨by decide, ?_, ⟨Z_on_qubit2, Z_on_qubit2_represents_nontrivial_coset,
    weight_Z_on_qubit2⟩⟩
  intro g _ hw
...
theorem repetitionCode3_min_weight_nontrivial_logical (g : NQubitPauliGroupElement 3)
    (hg : RepresentsNontrivialCoset g stabilizerGroup)
