...
-/
import QuantumMechanics.OneDimension.HilbertSpace.PositionStates
import QuantumMechanics.OneDimension.Operators.Unbounded
/-!
...
def parityOperatorSchwartz : 𝓢(ℝ, ℂ) →L[ℂ] 𝓢(ℝ, ℂ) := by
  ...
  · simpa using (contDiff_neg : ContDiff ℝ ⊤ (fun x : ℝ => -x))
  · intro n
