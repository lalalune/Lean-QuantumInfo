...
(In fact temperature is not really a fundamental choice; this remains open future work.)
...
-- NOTE (`LCSAY`): Make `SI : UnitChoices` computable, likely by replacing the
-- current axiom-defined units. See:
