...
/-!
# Ideal gas: basic entropy and adiabatic relations (macroscopic approach)

### Cross-reference: StatMech.IdealGas

`StatMech/IdealGas.lean` provides the complementary **microscopic** approach,
deriving the ideal gas law `PV = nRT` from the canonical partition function
via Gaussian integration. That module starts from the Hamiltonian and computes
the Helmholtz free energy; this module starts from the entropy function and
derives adiabatic relations. The two are linked by the Sackur-Tetrode equation.
