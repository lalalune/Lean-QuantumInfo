...
-/
import Relativity.Tensors.RealTensor.Vector.Basic
/-!
...
/-- The Minkowski product in coordinates: η_μν p^μ q^ν = p⁰q⁰ - ∑ᵢ pⁱqⁱ (in +--- signature). -/
...
lemma minkowskiProduct_invariant {d : ℕ} (p q : Vector d) (Λ : LorentzGroup d) :
    ⟪Λ • p, Λ • q⟫ₘ = ⟪p, q⟫ₘ := by
  rw [minkowskiProduct_toCoord, minkowskiProduct_toCoord]
  let pC : Lorentz.Contr d := Lorentz.ContrMod.toFin1dℝEquiv.symm p
  let qC : Lorentz.Contr d := Lorentz.ContrMod.toFin1dℝEquiv.symm q
  have hΛ : (Λ.1 : Matrix (Fin 1 ⊕ Fin d) (Fin 1 ⊕ Fin d) ℝ) ∈ LorentzGroup d := by
    simp [Λ.property]
  have hInv := (LorentzGroup.mem_iff_invariant (d := d) (Λ := Λ.1)).mp hΛ qC pC
  simpa [pC, qC, Lorentz.contrContrContractField.as_sum, Lorentz.ContrMod.mulVec_val,
    Lorentz.ContrMod.toFin1dℝ, Lorentz.Vector.smul_eq_mulVec] using hInv

open InnerProductSpace in
theorem minkowskiProduct_eq_timeComponent_spatialPart {d : ℕ} (p q : Vector d) :
    ⟪p, q⟫ₘ = p.timeComponent * q.timeComponent -
      ⟪p.spatialPart, q.spatialPart⟫_ℝ := by
  rw [minkowskiProduct_toCoord]
  simp only [Vector.timeComponent, Vector.spatialPart, PiLp.inner_apply, RCLike.inner_apply,
    conj_trivial]
  congr 1
  apply Finset.sum_congr rfl
  intro i _
  change p (Sum.inr i) * q (Sum.inr i) = q (Sum.inr i) * p (Sum.inr i)
  ring
