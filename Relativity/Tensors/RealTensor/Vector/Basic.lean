...
-/
import Relativity.Tensors.RealTensor.Metrics.Basic
import Relativity.Tensors.Elab
...
lemma differentiable_apply {d : ℕ} {α : Type*} [NormedAddCommGroup α] [NormedSpace ℝ α]
  ...
  · intro h ν
    ...
    · simpa using (Lorentz.Vector.coordCLM ν).differentiable
    · exact h
...
lemma contDiff_apply {n : WithTop ℕ∞} {d : ℕ} {α : Type*}
  ...
  · intro h ν
    ...
    apply ContDiff.comp
    · simpa using (Lorentz.Vector.coordCLM ν).contDiff
    · exact h
...
lemma fderiv_apply {d : ℕ} {α : Type*}
  ...
  rw [fderiv_comp _ (by simpa using (Lorentz.Vector.coordCLM ν).differentiableAt) (by exact h x)]
