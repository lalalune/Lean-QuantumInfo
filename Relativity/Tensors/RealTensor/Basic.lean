...
-/
import Relativity.Tensors.RealTensor.Metrics.Pre
import Relativity.Tensors.Contraction.Basis
/-!
...
theorem contrT_basis_repr_apply_eq_dropPairSection {n d: ℕ}
    {c : Fin (n + 1 + 1) → realLorentzTensor.Color}
    {i j : Fin (n + 1 + 1)} (h : i ≠ j ∧ (realLorentzTensor d).τ (c i) = c j)
    (h_contrT_basis_repr_apply_eq_dropPairSection :
      ∀ (t : ℝT(d, c)) (b : ComponentIdx (c ∘ Pure.dropPairEmb i j)),
        (Tensor.basis (c ∘ Pure.dropPairEmb i j)).repr (contrT n i j h t) b =
        ∑ (x : b.DropPairSection),
          ((Tensor.basis c).repr t x.1) *
          if (x.1 i).1 = (x.1 j).1 then 1 else 0)
    (t : ℝT(d, c)) (b : ComponentIdx (c ∘ Pure.dropPairEmb i j)) :
    (Tensor.basis (c ∘ Pure.dropPairEmb i j)).repr (contrT n i j h t) b =
    ∑ (x : b.DropPairSection),
    ((Tensor.basis c).repr t x.1) *
    if (x.1 i).1 = (x.1 j).1 then 1 else 0 :=
  h_contrT_basis_repr_apply_eq_dropPairSection t b

open ComponentIdx in
theorem contrT_basis_repr_apply_eq_fin {n d: ℕ} {c : Fin (n + 1 + 1) → realLorentzTensor.Color}
    {i j : Fin (n + 1 + 1)}
    {h : i ≠ j ∧ (realLorentzTensor d).τ (c i) = c j}
    (h_contrT_basis_repr_apply_eq_fin :
      ∀ (t : ℝT(d,c)) (b : ComponentIdx (c ∘ Pure.dropPairEmb i j)),
        (Tensor.basis (c ∘ Pure.dropPairEmb i j)).repr (contrT n i j h t) b =
        ∑ (x : Fin (1 + d)),
          ((Tensor.basis c).repr t
          (DropPairSection.ofFinEquiv h.1 b ⟨Fin.cast (by simp) x, Fin.cast (by simp) x⟩)))
    (t : ℝT(d,c)) (b : ComponentIdx (c ∘ Pure.dropPairEmb i j)) :
    (Tensor.basis (c ∘ Pure.dropPairEmb i j)).repr (contrT n i j h t) b =
    ∑ (x : Fin (1 + d)),
    ((Tensor.basis c).repr t
    (DropPairSection.ofFinEquiv h.1 b ⟨Fin.cast (by simp) x, Fin.cast (by simp) x⟩)) :=
  h_contrT_basis_repr_apply_eq_fin t b
