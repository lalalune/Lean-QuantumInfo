...
/-- The exponential of an element of the Lorentz algebra is in the connected
component of the identity. -/
lemma exp_mem_connectedComponent_id (A : lorentzAlgebra) :
    (⟨NormedSpace.exp A.1, exp_mem_lorentzGroup A⟩ : LorentzGroup 3) ∈
      connectedComponent (1 : LorentzGroup 3) := by
  ...
  have h_joined : Joined (1 : LorentzGroup 3) ⟨NormedSpace.exp A.1, exp_mem_lorentzGroup A⟩ :=
    ⟨exp_γ⟩
  exact pathComponent_subset_component _ h_joined
...
theorem exp_isProper (A : lorentzAlgebra) :
  ...
  have h_connected := exp_mem_connectedComponent_id A
  rw [← LorentzGroup.isProper_on_connected_component h_connected]
  exact LorentzGroup.id_isProper
