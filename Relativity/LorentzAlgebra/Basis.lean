...
-/
import Relativity.LorentzAlgebra.Basic
/-!
...
The next step is proving linear independence and spanning of these six generators,
then constructing a formal `Basis (Fin 2 × Fin 3) ℝ lorentzAlgebra`.
...
The structural properties documented above can be proved directly from the explicit
matrix definitions.
-/
...
lemma boostGenerator_transpose (i : Fin 3) :
  ...
  ext μ ν
  simp [boostGenerator, Matrix.transpose_apply, and_left_comm, and_assoc, or_left_comm, or_assoc,
    eq_comm]

@[simp]
lemma boostGenerator_trace (i : Fin 3) :
    Matrix.trace (boostGenerator i) = 0 := by
  simp [Matrix.trace, boostGenerator]
...
lemma rotationGenerator_transpose (i : Fin 3) :
  ...
  ext μ ν
  rcases μ with μ | μ <;> rcases ν with ν | ν
  · simp [Matrix.transpose_apply, rotationGenerator]
  · simp [Matrix.transpose_apply, rotationGenerator]
  · simp [Matrix.transpose_apply, rotationGenerator]
  · fin_cases i <;> fin_cases μ <;> fin_cases ν <;> simp [Matrix.transpose_apply, rotationGenerator]

@[simp]
lemma rotationGenerator_trace (i : Fin 3) :
    Matrix.trace (rotationGenerator i) = 0 := by
  fin_cases i <;> simp [Matrix.trace, rotationGenerator]
