...
-/
-- NOTE (`6VZHM`): Define the Lie group structure on the Lorentz group.
...
def toGL : LorentzGroup d →* GL (Fin 1 ⊕ Fin d) ℝ where
  toFun A := ⟨A.1, (A⁻¹).1, Matrix.mul_eq_one_comm.mpr $ mem_iff_dual_mul_self.mp A.2,
