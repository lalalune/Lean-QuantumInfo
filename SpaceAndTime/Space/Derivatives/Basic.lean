...
lemma deriv_component_sq {d : ℕ} {ν μ : Fin d} (x : Space d) :
  ...
  have hμ : DifferentiableAt ℝ (fun y : Space d => y μ) x :=
    (Space.eval_differentiable μ).differentiableAt
  have hsmul := (deriv_smul (u := ν) (x := x)
    (c := fun y : Space d => y μ) (f := fun y : Space d => y μ) hμ hμ)
  by_cases hνμ : ν = μ
  · subst hνμ
    simpa [pow_two, smul_eq_mul, deriv_component, two_mul] using hsmul
  · simpa [pow_two, smul_eq_mul, deriv_component, hνμ] using hsmul
...
lemma norm_sq_differentiable : Differentiable ℝ (fun x : Space d => ‖x‖ ^ 2) := by
  intro x
  simpa using (HasFDerivAt.norm_sq
    (f := fun y : Space d => y)
    (f' := (ContinuousLinearMap.id ℝ (Space d)))
    (x := x)
...
lemma deriv_norm_sq (x : Space d) (i : Fin d) :
  ...
  rw [deriv_eq_fderiv_basis]
  have hnorm : HasFDerivAt (fun y : Space d => ‖y‖ ^ 2)
      (2 • (innerSL ℝ x)) x := by
    simpa using (HasFDerivAt.norm_sq
      (f := fun y : Space d => y)
      (f' := (ContinuousLinearMap.id ℝ (Space d)))
      ...
      (hasFDerivAt_id x))
  rw [hnorm.fderiv]
  simp [Space.inner_basis, two_mul]
...
lemma inner_differentiable {d : ℕ} :
  ...
  simpa [real_inner_self_eq_norm_sq] using (norm_sq_differentiable (d := d))
...
lemma schwartMap_fderiv_comm {d}
    ...
    fderiv ℝ (fun x => fderiv ℝ η x (basis ν)) x (basis μ) =
    fderiv ℝ (fun x => fderiv ℝ η x (basis μ)) x (basis ν) := by
  have hfdiff : DifferentiableAt ℝ (fderiv ℝ η) x := by
    exact (((η.smooth 2).contDiffAt).fderiv_right (m := 1) (by norm_num)).differentiableAt
      (by norm_num)
  have hμ :
      fderiv ℝ (fun x => fderiv ℝ η x (basis ν)) x (basis μ) =
        fderiv ℝ (fderiv ℝ η) x (basis μ) (basis ν) := by
    simp [fderiv_clm_apply, hfdiff]
  have hν :
      fderiv ℝ (fun x => fderiv ℝ η x (basis μ)) x (basis ν) =
        fderiv ℝ (fderiv ℝ η) x (basis ν) (basis μ) := by
    simp [fderiv_clm_apply, hfdiff]
  have hsymm :
      fderiv ℝ (fderiv ℝ η) x (basis μ) (basis ν) =
        fderiv ℝ (fderiv ℝ η) x (basis ν) (basis μ) := by
    exact ((η.smooth 2).contDiffAt).isSymmSndFDerivAt (by simp) (basis μ) (basis ν)
  exact hμ.trans (hsymm.trans hν.symm)
