...
def slice {d} (i : Fin d.succ) : Space d.succ ≃L[ℝ] ℝ × Space d where
  ...
  continuous_toFun := by
    ...
    · refine Space.mk_continuous.comp ?_
      ...
      intro j
      exact Space.eval_continuous (Fin.succAbove i j)
  continuous_invFun := by
    refine Space.mk_continuous.comp ?_
    refine continuous_pi ?_
    intro j
    rcases Fin.eq_self_or_eq_succAbove i j with rfl | ⟨k, rfl⟩
    · simpa using (continuous_fst : Continuous fun p : ℝ × Space d => p.1)
    · simpa using (Space.eval_continuous k).comp
        (continuous_snd : Continuous fun p : ℝ × Space d => p.2)
...
lemma slice_symm_measurableEmbedding {d : ℕ} (i : Fin d.succ) :
  ...
  exact Homeomorph.measurableEmbedding ((slice i).symm.toHomeomorph)
...
lemma fderiv_slice_symm_left_apply {d : ℕ} (i : Fin d.succ) (x : Space d) (r1 r2 : ℝ) :
  ...
  rw [fderiv_comp',
    DifferentiableAt.fderiv_prodMk (by
      simpa using (differentiableAt_const (c := x) : DifferentiableAt ℝ (fun _ : ℝ => x) r1))]
...
lemma fderiv_slice_symm_right_apply {d : ℕ} (i : Fin d.succ) (r : ℝ)
  ...
  rw [fderiv_comp',
    DifferentiableAt.fderiv_prodMk (by
      simpa using (differentiableAt_const (c := r) : DifferentiableAt ℝ (fun _ : Space d => r) x1))]
...
lemma fderiv_fun_slice_symm_right_apply {d : ℕ} (i : Fin d.succ) (r : ℝ)
  ...
  rw [fderiv_comp' _ _ (by
    have hpair : DifferentiableAt ℝ (fun x : Space d => (r, x)) x1 := by
      fun_prop
    exact (slice i).symm.differentiableAt.comp x1 hpair)]
...
lemma fderiv_fun_slice_symm_left_apply {d : ℕ} (i : Fin d.succ) (r1 r2 : ℝ)
  ...
  rw [fderiv_comp' _ _ (by
    have hpair : DifferentiableAt ℝ (fun r : ℝ => (r, x)) r1 := by
      fun_prop
    exact (slice i).symm.differentiableAt.comp r1 hpair)]
  simp only [Nat.succ_eq_add_one, ContinuousLinearMap.coe_comp', Function.comp_apply,
