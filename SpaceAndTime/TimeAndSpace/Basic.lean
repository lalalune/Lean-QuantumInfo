...
lemma curl_differentiable_time
  ...
  · simp [curl, Fin.isValue]
    ...
    · exact space_deriv_differentiable_time
        (f := fun t x' => fₜ t x' 2) (hf := by fun_prop) x
    · exact space_deriv_differentiable_time
        (f := fun t x' => fₜ t x' 1) (hf := by fun_prop) x
  · simp [curl, Fin.isValue]
    refine Differentiable.sub ?_ ?_
    · exact space_deriv_differentiable_time
        (f := fun t x' => fₜ t x' 0) (hf := by fun_prop) x
    · exact space_deriv_differentiable_time
        (f := fun t x' => fₜ t x' 2) (hf := by fun_prop) x
  · simp [curl, Fin.isValue]
    refine Differentiable.sub ?_ ?_
...
lemma time_deriv_curl_commute (fₜ : Time → Space → EuclideanSpace ℝ (Fin 3))
  ...
  ext i
  have hdiff_t : ∀ x', Differentiable ℝ (fun t' => fₜ t' x') := by
    intro x'
    have huncurry : Differentiable ℝ ↿fₜ := hf.differentiable (by simp)
    simpa [Function.comp] using
      (huncurry.comp (by fun_prop : Differentiable ℝ (fun t' : Time => (t', x'))))
  fin_cases i
  · have hdiff1 : Differentiable ℝ (fun t' => Space.deriv 1 (fₜ t' · 2) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 2) (hf := by fun_prop) x
    have hdiff2 : Differentiable ℝ (fun t' => Space.deriv 2 (fₜ t' · 1) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 1) (hf := by fun_prop) x
    have hcoord2 : ∀ x', ∂ₜ (fun t' => fₜ t' x' 2) t = (∂ₜ (fun t' => fₜ t' x') t) 2 := by
      intro x'
      exact Time.deriv_euclid (μ := 2) (f := fun t' => fₜ t' x') (hf := hdiff_t x') (t := t)
    have hcoord1 : ∀ x', ∂ₜ (fun t' => fₜ t' x' 1) t = (∂ₜ (fun t' => fₜ t' x') t) 1 := by
      intro x'
      exact Time.deriv_euclid (μ := 1) (f := fun t' => fₜ t' x') (hf := hdiff_t x') (t := t)
    calc
      (∂ₜ (fun t' => (∇ × fₜ t') x) t) 0
          = ∂ₜ (fun t' => ((∇ × fₜ t') x) 0) t := by
              symm
              exact Time.deriv_euclid
                (hf := curl_differentiable_time fₜ hf x) t
      _ = ∂ₜ (fun t' => Space.deriv 1 (fₜ t' · 2) x - Space.deriv 2 (fₜ t' · 1) x) t := by
            simp [curl, Fin.isValue]
      _ = ∂ₜ (fun t' => Space.deriv 1 (fₜ t' · 2) x) t
            - ∂ₜ (fun t' => Space.deriv 2 (fₜ t' · 1) x) t := by
            rw [Time.deriv_eq, fderiv_fun_sub]
            · simp [Time.deriv_eq]
            · exact hdiff1.differentiableAt
            · exact hdiff2.differentiableAt
      _ = Space.deriv 1 (fun x' => ∂ₜ (fun t' => fₜ t' x' 2) t) x
            - Space.deriv 2 (fun x' => ∂ₜ (fun t' => fₜ t' x' 1) t) x := by
            congr
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 2) (hf := by fun_prop) t x
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 1) (hf := by fun_prop) t x
      _ = ((∇ × fun x' => (∂ₜ (fun t' => fₜ t' x') t)) x) 0 := by
            simp [curl, Fin.isValue, hcoord2, hcoord1]
  · have hdiff1 : Differentiable ℝ (fun t' => Space.deriv 2 (fₜ t' · 0) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 0) (hf := by fun_prop) x
    have hdiff2 : Differentiable ℝ (fun t' => Space.deriv 0 (fₜ t' · 2) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 2) (hf := by fun_prop) x
    have hcoord0 : ∀ x', ∂ₜ (fun t' => fₜ t' x' 0) t = (∂ₜ (fun t' => fₜ t' x') t) 0 := by
      intro x'
      exact Time.deriv_euclid (μ := 0) (f := fun t' => fₜ t' x') (hf := hdiff_t x') (t := t)
    have hcoord2 : ∀ x', ∂ₜ (fun t' => fₜ t' x' 2) t = (∂ₜ (fun t' => fₜ t' x') t) 2 := by
      intro x'
      exact Time.deriv_euclid (μ := 2) (f := fun t' => fₜ t' x') (hf := hdiff_t x') (t := t)
    calc
      (∂ₜ (fun t' => (∇ × fₜ t') x) t) 1
          = ∂ₜ (fun t' => ((∇ × fₜ t') x) 1) t := by
              symm
              exact Time.deriv_euclid
                (hf := curl_differentiable_time fₜ hf x) t
      _ = ∂ₜ (fun t' => Space.deriv 2 (fₜ t' · 0) x - Space.deriv 0 (fₜ t' · 2) x) t := by
            simp [curl, Fin.isValue]
      _ = ∂ₜ (fun t' => Space.deriv 2 (fₜ t' · 0) x) t
            - ∂ₜ (fun t' => Space.deriv 0 (fₜ t' · 2) x) t := by
            rw [Time.deriv_eq, fderiv_fun_sub]
            · simp [Time.deriv_eq]
            · exact hdiff1.differentiableAt
            · exact hdiff2.differentiableAt
      _ = Space.deriv 2 (fun x' => ∂ₜ (fun t' => fₜ t' x' 0) t) x
            - Space.deriv 0 (fun x' => ∂ₜ (fun t' => fₜ t' x' 2) t) x := by
            congr
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 0) (hf := by fun_prop) t x
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 2) (hf := by fun_prop) t x
      _ = ((∇ × fun x' => (∂ₜ (fun t' => fₜ t' x') t)) x) 1 := by
            simp [curl, Fin.isValue, hcoord0, hcoord2]
  · have hdiff1 : Differentiable ℝ (fun t' => Space.deriv 0 (fₜ t' · 1) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 1) (hf := by fun_prop) x
    have hdiff2 : Differentiable ℝ (fun t' => Space.deriv 1 (fₜ t' · 0) x) :=
      space_deriv_differentiable_time (f := fun t' x' => fₜ t' x' 0) (hf := by fun_prop) x
    have hcoord1 : ∀ x', ∂ₜ (fun t' => fₜ t' x' 1) t = (∂ₜ (fun t' => fₜ t' x') t) 1 := by
      intro x'
      exact Time.deriv_euclid (μ := 1) (f := fun t' => fₜ t' x') (hf := hdiff_t x') (t := t)
    ...
    calc
      (∂ₜ (fun t' => (∇ × fₜ t') x) t) 2
          = ∂ₜ (fun t' => ((∇ × fₜ t') x) 2) t := by
              symm
              exact Time.deriv_euclid
                (hf := curl_differentiable_time fₜ hf x) t
      _ = ∂ₜ (fun t' => Space.deriv 0 (fₜ t' · 1) x - Space.deriv 1 (fₜ t' · 0) x) t := by
            simp [curl, Fin.isValue]
      _ = ∂ₜ (fun t' => Space.deriv 0 (fₜ t' · 1) x) t
            - ∂ₜ (fun t' => Space.deriv 1 (fₜ t' · 0) x) t := by
            rw [Time.deriv_eq, fderiv_fun_sub]
            · simp [Time.deriv_eq]
            · exact hdiff1.differentiableAt
            · exact hdiff2.differentiableAt
      _ = Space.deriv 0 (fun x' => ∂ₜ (fun t' => fₜ t' x' 1) t) x
            - Space.deriv 1 (fun x' => ∂ₜ (fun t' => fₜ t' x' 0) t) x := by
            congr
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 1) (hf := by fun_prop) t x
            · exact time_deriv_comm_space_deriv
                (f := fun t' x' => fₜ t' x' 0) (hf := by fun_prop) t x
      _ = ((∇ × fun x' => (∂ₜ (fun t' => fₜ t' x') t)) x) 2 := by
            simp [curl, Fin.isValue, hcoord1, hcoord0]
...
lemma distTimeDeriv_apply {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  simp [distTimeDeriv, Distribution.fderivD]

lemma distTimeDeriv_apply' {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
    (f : (Time × Space d) →d[ℝ] M) (ε : 𝓢(Time × Space d, ℝ)) :
    (distTimeDeriv f) ε = - fderivD ℝ f ε (1, 0) := by
  rw [distTimeDeriv_apply, Distribution.fderivD_apply]
  simp

lemma apply_fderiv_eq_distTimeDeriv {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
    (f : (Time × Space d) →d[ℝ] M) (ε : 𝓢(Time × Space d, ℝ)) :
    fderivD ℝ f ε (1, 0) = - (distTimeDeriv f) ε := by
  rw [distTimeDeriv_apply, Distribution.fderivD_apply]
  simp
...
lemma distTimeDeriv_apply_CLM {M M2 d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  ext ε
  simp [distTimeDeriv_apply, Distribution.fderivD_apply]
...
noncomputable def distSpaceDeriv {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  toFun f :=
    let ev : (Time × Space d →L[ℝ] M) →L[ℝ] M := {
      toFun v := v (0, basis i)
      map_add' v1 v2 := by simp only [ContinuousLinearMap.add_apply]
...
lemma distSpaceDeriv_apply {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  simp [distSpaceDeriv, Distribution.fderivD]

lemma distSpaceDeriv_apply' {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
    (i : Fin d) (f : (Time × Space d) →d[ℝ] M) (ε : 𝓢(Time × Space d, ℝ)) :
    (distSpaceDeriv i f) ε = - fderivD ℝ f ε (0, basis i) := by
  rw [distSpaceDeriv_apply, Distribution.fderivD_apply]
  simp

lemma apply_fderiv_eq_distSpaceDeriv {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
    (i : Fin d) (f : (Time × Space d) →d[ℝ] M) (ε : 𝓢(Time × Space d, ℝ)) :
    fderivD ℝ f ε (0, basis i) = - (distSpaceDeriv i f) ε := by
  rw [distSpaceDeriv_apply, Distribution.fderivD_apply]
  simp
...
lemma distSpaceDeriv_commute {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  ext ε
  simp [distSpaceDeriv_apply, Distribution.fderivD_apply]
...
lemma distTimeDeriv_commute_distSpaceDeriv {M d} [NormedAddCommGroup M] [NormedSpace ℝ M]
  ...
  ext ε
  simp [distTimeDeriv_apply, distSpaceDeriv_apply, Distribution.fderivD_apply]
...
noncomputable def distSpaceGrad {d} :
  ...
  toFun f :=
    let gradLin : (Time × Space d →L[ℝ] ℝ) →ₗ[ℝ] EuclideanSpace ℝ (Fin d) := {
      toFun dfdx := WithLp.toLp 2 fun i => dfdx (0, basis i)
      map_add' v1 v2 := by
        ext i
        simp
      map_smul' a v := by
    ...
    let grad : (Time × Space d →L[ℝ] ℝ) →L[ℝ] EuclideanSpace ℝ (Fin d) :=
      ⟨gradLin, LinearMap.continuous_of_finiteDimensional gradLin⟩
    grad.comp (Distribution.fderivD ℝ f)
  map_add' f1 f2 := by
    ext ε i
    simp [Distribution.fderivD]
  map_smul' a f := by
...
lemma distSpaceGrad_apply {d} (f : (Time × Space d) →d[ℝ] ℝ) (ε : 𝓢(Time × Space d, ℝ)) :
  ...
  ext i
  simp [distSpaceGrad, distSpaceDeriv, Distribution.fderivD]
...
noncomputable def distSpaceDiv {d} :
  ...
  toFun f :=
    let divLin : (Time × Space d →L[ℝ] EuclideanSpace ℝ (Fin d)) →ₗ[ℝ] ℝ := {
      toFun dfdx := ∑ i, dfdx (0, basis i) i
      map_add' v1 v2 := by
        simp [Finset.sum_add_distrib]
      map_smul' a v := by
        simp [Finset.mul_sum]
    }
    let div : (Time × Space d →L[ℝ] EuclideanSpace ℝ (Fin d)) →L[ℝ] ℝ :=
      ⟨divLin, LinearMap.continuous_of_finiteDimensional divLin⟩
    div.comp (Distribution.fderivD ℝ f)
  map_add' f1 f2 := by
    ext ε
    simp [Distribution.fderivD]
  map_smul' a f := by
...
lemma distSpaceDiv_apply_eq_sum_distSpaceDeriv {d}
    ...
    distSpaceDiv f η = ∑ i, distSpaceDeriv i f η i := by
  simp [distSpaceDiv, distSpaceDeriv, Distribution.fderivD]
