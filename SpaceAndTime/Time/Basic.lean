...
lemma val_measurable : Measurable Time.val := by
  simpa using toRealCLE.continuous.measurable

lemma val_measurableEmbedding : MeasurableEmbedding Time.val where
  injective := val_injective
  measurable := val_measurable
  measurableSet_image' := by
    intro s hs
    change MeasurableSet (⇑toRealCLE '' s)
    rw [Set.image_eq_preimage_of_inverse toRealCLE.symm_apply_apply toRealCLE.apply_symm_apply]
